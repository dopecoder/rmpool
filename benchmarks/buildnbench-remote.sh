...
LD_PRELOAD=../preloadlib/bin/preloadlib.so sysbench --test=memory --memory-block-size=64K --memory-total-size=100G --num-threads=1 run > ./data/sysbench/sysbench-64K-remote

LD_PRELOAD=../preloadlib/bin/preloadlib.so sysbench --test=memory --memory-block-size=1M --memory-total-size=100G --num-threads=1 run > ./data/sysbench/sysbench-1M-remote

LD_PRELOAD=../preloadlib/bin/preloadlib.so sysbench --test=memory --memory-block-size=4M --memory-total-size=100G --num-threads=1 run > ./data/sysbench/sysbench-4M-remote
